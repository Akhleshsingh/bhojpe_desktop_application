      ...
      <Dialog open={ebillPopup} onClose={()=>setEbillPopup(false)} PaperProps={{sx:{width:420,maxWidth:"96vw",borderRadius:"14px",overflow:"hidden"}}}>
        ...
        <Box sx={{px:"18px",py:"13px",borderTop:`1px solid ${C.bd}`,display:"flex",gap:"8px",background:C.s1}}>
          ...
          <Box component="button" onClick={()=>{
            if(!ebillName||!ebillPhone){toast.error("Naam aur number required!");return;}
            // Read restaurant info from saved bill settings
            let restName="Restaurant", restPhone="", restAddr="";
            try {
              const bs = JSON.parse(localStorage.getItem("bhojpe_bill_settings")||"{}");
              restName = bs.restaurant_name || branchData?.data?.restaurant?.name || "Restaurant";
              restPhone = bs.phone || "";
              restAddr  = bs.address || "";
            } catch {}
            // Clean customer phone — strip non-digits, prepend 91 if 10-digit Indian number
            const digits = ebillPhone.replace(/\D/g,"");
            const waPhone = digits.length===10 ? `91${digits}` : digits;
            // Format date
            const now = new Date();
            const dateStr = `${String(now.getDate()).padStart(2,"0")}/${String(now.getMonth()+1).padStart(2,"0")}/${String(now.getFullYear()).slice(-2)}  ${now.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}`;
            // Build bill lines
            const itemLines = cart.map(i=>`  • ${i.qty}× ${i.item.item_name||i.item.name||"Item"}${i.variationName?` (${i.variationName})`:""} — ₹${i.qty*i.price}`).join("\n");
            const orderType = channel==="dine"?"Dine In":channel==="pickup"?"Pickup":"Delivery";
            const tableInfo = assignedTable ? `Table ${assignedTable.table_no}` : orderType;
            const deliveryLine = channel==="delivery"&&delCharge>0?`\nDelivery Charge: ₹${delCharge}`:"";
            const discLine = discAmt>0?`\nDiscount: -₹${discAmt}`:"";
            const msg =
`🏪 *${restName}*${restAddr?`\n📍 ${restAddr}`:""}

📋 *BILL RECEIPT*
🔖 Order #${String(orderNo).padStart(3,"0")} | ${tableInfo}${pax>1?` | Pax: ${pax}`:""}
📅 ${dateStr} | ${orderType}
...
━━━━━━━━━━━━━━━━━
Subtotal: ₹${subtotal}
GST (5%): ₹${tax}${discLine}${deliveryLine}
*TOTAL: ₹${Math.round(total)}*
💳 Payment: ${payMode}
━━━━━━━━━━━━━━━━━

Thank you, *${ebillName}*! 🙏
Visit us again!${restPhone?`\n📞 ${restPhone}`:""}`;
            window.open(`https://wa.me/${waPhone}?text=${encodeURIComponent(msg)}`,"_blank");
            toast.success(`Opening WhatsApp for ${ebillName} ✓`);
            setEbillPopup(false);
          }} sx={{flex:1,py:"10px",background:"#25d366",border:"none",borderRadius:"10px",color:"#fff",fontFamily:FONT,fontWeight:800,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px",boxShadow:"0 2px 8px rgba(37,211,102,.28)"}}>
